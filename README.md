...
Keep the official OpenAI Python client and the appointment workflow; point `base_url` at Infrai, use `model="auto"`, and make the patient-safety decision before a prompt can leave the service. This is the narrow migration boundary: one `INFRAI_API_KEY` reaches the OpenAI-compatible endpoint while the existing `chat.completions.create(...)` call shape stays familiar.
